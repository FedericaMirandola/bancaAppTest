...
public class TransactionControllerTest {
        ...
        public void setUp() {
                ...
                Transaction freshUndefinedTransaction = new Transaction();
                freshUndefinedTransaction.setId(3L);
                freshUndefinedTransaction.setAccountId("testAccountId");
                freshUndefinedTransaction.setTransactionId("trans_id_undefined");
                freshUndefinedTransaction.setBookingDate(OffsetDateTime.parse("2023-11-01T09:00:00Z"));
                freshUndefinedTransaction.setAmount(BigDecimal.valueOf(25.0));
                freshUndefinedTransaction.setRemittanceInformation("Transazione Sconosciuta");
                freshUndefinedTransaction.setCenterType(CenterType.UNDEFINED);
                freshUndefinedTransaction.setIsManuallyClassified(false);
                freshUndefinedTransaction.setCurrency("EUR");

                when(mockQueryFound.firstResult()).thenReturn(freshUndefinedTransaction);
                when(transactionRepository.find(eq("transactionId"), eq("trans_id_undefined")))
                                .thenReturn(mockQueryFound);

                // 2. Mock per find("transactionId", "non_esistente") che NON trova la
                // transazione
                // firstResult() lancia NoResultException in questo caso, che il controller
                // cattura come 500.
                PanacheQuery<Transaction> mockQueryNotFound = Mockito.mock(PanacheQuery.class);
                when(mockQueryNotFound.firstResult())
                                .thenThrow(new jakarta.persistence.NoResultException("No transaction found for ID"));
                when(transactionRepository.find(eq("transactionId"), eq("non_esistente")))
                                .thenReturn(mockQueryNotFound);

                // 3. Mock per find("transactionId", "trans_id_error") che simula un errore nel
                // DB
                PanacheQuery<Transaction> mockQueryError = Mockito.mock(PanacheQuery.class);
                when(mockQueryError.firstResult())
        ...
        void transactions_shouldReturnTransactions_whenNoQueryParams() {
                ...
                given()
                                ...
                                .body("[1].remittanceInformation",
                                                is(transactionTestProfitto.getRemittanceInformation()));
        ...
        void transactions_shouldReturnFilteredTransactions_whenDateRangeProvided() {
                ...
                verify(transactionService, times(1)).searchTransactions(eq(DEFAULT_TEST_ACCOUNT_ID), eq(fromDate),
                                eq(toDate),
                                eq(null));
        }
        ...
        // ------ transaction/manual-classify test -------
       ...
       /*  @Test
        void manualClassifyTransaction_shouldUpdateCenterTypeAndSetManualFlag_whenValid() {
                ...
                // argThat. Nota: argThat verificherà lo stato dell'oggetto *dopo* le modifiche
                // nel controller.
        ...
        @Test
        void manualClassifyTransaction_shouldReturnInternalServerError_whenTransactionDoesNotExist() {
                given()
                                .auth().basic(VALID_USERNAME, VALID_PASSWORD)
                                .contentType(ContentType.JSON)
                                .queryParam("centerType", "COSTO")
                                .when()
                                .put("/transactions/{transactionId}/manual-classify", "non_esistente")
                                .then()
                                .statusCode(500)
                                .body(containsString("Errore durante la classificazione manuale"));
        ...
        void manualClassifyTransaction_shouldReturnBadRequest_whenCenterTypeIsMissing() {
                given()
                                ...
                                .then()
                                .statusCode(400)
                                .body(is("Parametro centerType obbligatorio."));

                verify(transactionRepository, never()).find("transactionId", "trans_id_undefined"); // Nessuna
                                                                                                    // interazione col
                                                                                                    // DB
        ...
        void manualClassifyTransaction_shouldReturnBadRequest_whenInvalidCenterType() {
                given()
                                ...
                                .then()
                                .statusCode(400)
                                .body(containsString("Tipo centro non valido. Tipi accettati: COSTO o PROFITTO"));
        ...
        void manualClassifyTransaction_shouldReturnBadRequest_whenCenterTypeIsUndefined() {
                given()
                                ...
                                .statusCode(400)
                                .body(containsString(
                                                "Non si può impostare manualmente il cento UNDEFINED"));
        ...
        void manualClassifyTransaction_shouldReturnInternalServerError_whenRepositoryFails() {
                given()
                                ...
                                .statusCode(500)
                                .body(containsString(
                                                "Errore durante la classificazione manuale"));
